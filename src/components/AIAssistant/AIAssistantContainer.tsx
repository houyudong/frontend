...
interface AIAssistantContainerProps {
  context?: string;
  type?: 'general' | 'challenge' | 'thought';
  thoughtQuestion?: string;
  challengeData?: ChallengeData | null;
  hideContextTip?: boolean;
  mode?: 'conversation' | 'codeGenerator';
  initialPromptSelector?: string | null;
}
 ...
 * @component
 */
const AIAssistantContainer: React.FC<AIAssistantContainerProps> = ({ 
  context, 
  type = 'general', 
  thoughtQuestion, 
  challengeData = null, 
  ...
  useEffect(() => {
    if (mode === 'codeGenerator' && initialPromptSelector) {
      // 设置监听器以捕获文本区域变化
      const promptElement = document.querySelector(initialPromptSelector) as HTMLInputElement | HTMLTextAreaElement;
      if (promptElement) {
          ...
          {showResultsForm && (
            <form onSubmit={handleSubmitResults} className="mt-4">
              ...
              <textarea
                ...
                rows={3}
                placeholder="请输入您的实验结果和反馈..."
              />
              <button
                type="submit"
                className="mt-2 px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 flex items-center"
              >
