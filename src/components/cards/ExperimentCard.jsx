...
const ExperimentCard = ({
  ...
  // 将难度级别转换为中文（如果是英文）
  const getChineseDifficulty = (level) => {
    switch(level.toLowerCase()) {
      case 'beginner':
        return '初级';
      case 'intermediate':
  ...
  const getIconType = (experimentType) => {
  ...
  // 根据实验类型获取徽章颜色
  const getTypeBadgeColor = (experimentType) => {
        ...
        <div className="p-4 flex flex-col">
          ...
          <div className="experiment-meta">
            ...
            <div className="time-estimate">
              <FaClock className="mr-1" /> {formatTimeEstimate(timeEstimate)}
            </div>
          </div>
