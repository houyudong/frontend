...
/**
 * TeacherDashboardPage - 教师管理平台页面
 *
 * 提供教师查看学生活动记录和学习进度的功能。
 * 包括学生列表展示、活动记录查看等功能。
 *
 * @component
 * @example
 ...
 * ```
 *
 * @returns {JSX.Element} TeacherDashboardPage组件
 */
const TeacherDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchStudentActivities = async () => {
      try {
        setLoading(true);
        const data = await getStudentActivities(selectedStudent.id.toString());
        setStudentActivities(data.items);
        setLoading(false);
      } catch (err: any) {
                ...
                <ul className="divide-y divide-gray-200 max-h-screen overflow-y-auto">
                  {students.map((student) => (
                    <li key={student.id}>
                      ...
                      >
                        <div className="font-medium text-gray-800">{student.fullName || student.username || '未命名学生'}</div>
            ...
            <div className="w-full md:w-2/3">
              <div className="p-4 border-b border-gray-200 bg-gray-50">
                <h3 className="font-medium text-gray-700">
                  {selectedStudent
                    ? `${selectedStudent.fullName || selectedStudent.username || '学生'} (${selectedStudent.id}) 的活动记录`
                    : '请选择一名学生查看活动'}
                </h3>
              </div>
